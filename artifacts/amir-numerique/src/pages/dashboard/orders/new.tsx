...
import { useListServices, useGetMe, useCreateOrder } from "@workspace/api-client-react";
  ...
  return (
    <DashboardLayout>
      <div className="max-w-2xl">
        ...
        <h1 className="text-2xl font-bold mb-8">{t("newOrder")}</h1>
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            ...
            <FormField control={form.control} name="note" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea placeholder="Instructions spéciales, finitions, etc." rows={3} data-testid="textarea-note" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />

            <Button type="submit" className="w-full h-11" disabled={createOrder.isPending || !calcData || calcData.area === 0} data-testid="button-submit-order">
              {createOrder.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Envoi...</> : "Passer la commande"}
            </Button>
          </form>
        </Form>
      </div>
