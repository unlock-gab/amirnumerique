...
import { SidebarProvider, Sidebar, SidebarHeader, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarFooter, SidebarGroupLabel } from "@/components/ui/sidebar";
...
export function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const navItems = [
    ...
    { href: "/admin/services", label: t("manageServices"), icon: Printer },
        ...
        <Sidebar className="border-r border-border">
          <SidebarHeader className="border-b border-border/40 p-4">
            <Link href="/" className="flex items-center space-x-2">
              <span className="text-xl font-bold text-primary tracking-tight">Amir Admin</span>
            </Link>
          </SidebarHeader>
          <SidebarContent>
            <SidebarGroup>
          ...
          <SidebarFooter className="border-t border-border/40 p-4 flex flex-col gap-2">
            ...
            <div className="text-xs text-muted-foreground truncate mb-2">{user.email} (Admin)</div>
            <Button 
              variant="destructive" 
